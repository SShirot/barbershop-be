...
const AttributeService = {
  getAll: async (page, pageSize, name) => {
    ...
    const { data, total } = await AttributeRepository.getAll(
      offset,
      pageSize,
      name
    );
  ...
  getAllAttributeValue: async (page, pageSize, req) => {
    const offset = (page - 1) * pageSize;
    const { data, total } = await AttributeValueRepository.getAll(
      offset,
      pageSize,
      req
    );
  ...
  update: async (id, updateData) => {
    const isUpdated = await AttributeRepository.updateWithValues(
      id,
      updateData
    );
